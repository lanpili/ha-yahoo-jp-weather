...
1. [`dashboard/yahoo-weather-card.js`](dashboard/yahoo-weather-card.js) を次へコピーします。
   ...
   ```
2. [`dashboard/yahoo-weather-card-i18n`](dashboard/yahoo-weather-card-i18n) ディレクトリ全体を次へコピーします。
   ```text
   /config/www/yahoo-weather-card-i18n/
   ```
3. 「**設定 → ダッシュボード → リソース**」にJavaScriptモジュールを追加します。
   ```text
   /local/yahoo-weather-card.js?v=2.3.0
   ```
4. フロントエンドを再読み込みするか、Home Assistantアプリを完全に終了して開き直します。
...
翻訳は独立したJSON言語パックです。言語を追加する場合は`en.json`をコピーし、すべての値を翻訳し、`tabAliases`へHA標準の時間別・日別ラベルを設定し、16方位を維持して、`fr.json`のような小文字の基本言語コードで保存します。メインJavaScriptの変更は不要です。
