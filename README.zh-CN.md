...
这是一个非官方 Home Assistant 自定义集成，从 [Yahoo!天気・災害](https://weather.yahoo.co.jp/weather/) 及 Yahoo 天气 App 的一小时预报数据源获取日本市区町村天气预报。

> 本项目与 Yahoo Japan Corporation 没有隶属或官方合作关系。Yahoo 没有为此用途提供正式公开 API；本集成使用公开地区页面和 Yahoo 天气 App 使用的数据接口，因此上游发生变化时可能需要更新集成。
...
- 无需 API Key
- 当前天气采用最近的 Yahoo 1 小时预报时段
- 真实一小时预报：天气、温度、降水概率、湿度、降水量、风向和风速
- 最多 10 天每日预报：最高/最低温度、降水概率
- 可选的自适应仪表板增强，显示 Yahoo 风格逐小时详情
...
不安装此文件也不影响集成本体和 HA 标准天气卡。若希望标准 `weather-forecast` 卡根据高度显示逐小时详情：

1. 将 [`dashboard/yahoo-weather-card.js`](dashboard/yahoo-weather-card.js) 复制到：
   ```text
...
| 数据 | 来源 |
|---|---|
| 当前天气、温度 | 最近的 Yahoo 1 小时预报 |
| 每小时预报 | Yahoo 天气 App 的一小时预报 |
| 每日预报 | Yahoo 天气 App 每日预报，最多 10 天 |
| 湿度 | 一小时预报 |
| 降水 | 一小时降水量及概率；每日使用 Yahoo 公布的降水概率 |
| 风 | 一小时预报中的风向和风速 |
...
- Yahoo! JAPAN 没有为此用途提供正式公开 API。代码内置的 App 客户端标识不是用户 API Key。
- 集成依赖 Yahoo 当前地区页面结构和 App 预报响应格式。
- 请勿把 30 分钟更新间隔调得更短。
...
- 旧版 URL、页面中发现的链接和最终地区页只允许使用 `weather.yahoo.co.jp` 上的 HTTPS 市区町村页面；跨站重定向会被拒绝。
- 发布时间缺失或过旧、预报全部过期、响应无有效数据或数值明显异常时，本次更新会失败并保留旧数据。
- Home Assistant 每 30 分钟抓取时，Yahoo 可以看到您的公网 IP、所选市区町村代码、请求时间和集成 User-Agent。
- 集成不会保存下载的 HTML 或预报响应。Home Assistant 诊断不会包含市区町村名称、来源 URL 或具体预报内容。
